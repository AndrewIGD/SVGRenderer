...
class Rectangle(Drawable):
    ...
    def draw(self):
        ...
        image.arc([rect_x1 - self.rx * 2, rect_y1 - self.ry * 2, rect_x1, rect_y1],
                 start=0, end=90, fill=fill_color)
        image.arc([rect_x0, rect_y1 - self.ry * 2, rect_x0 + self.rx * 2, rect_y1],
                 start=90, end=180, fill=fill_color)

        image.line([rect_x0 + self.rx, rect_y0, rect_x1 - self.rx, rect_y0], fill=fill_color, width=1)
        image.line([rect_x1, rect_y0 + self.ry, rect_x1, rect_y1 - self.ry], fill=fill_color, width=1)
        image.line([rect_x0 + self.rx, rect_y1, rect_x1 - self.rx, rect_y1], fill=fill_color, width=1)
        image.line([rect_x0, rect_y0 + self.ry, rect_x0, rect_y1 - self.ry], fill=fill_color, width=1)